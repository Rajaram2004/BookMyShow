...
public class MovieService {
	...
	public MovieService(List<Movies> movie) {
		this.movies = movie;
	...
	private void initializeMovies() {
        ...
        movies.add(new Movies(1, "Inception", "2h 28m", "Sci-Fi", "English", Arrays.asList(101, 102)));
        movies.add(new Movies(2, "3 Idiots", "2h 45m", "Comedy", "Hindi", Arrays.asList(101, 103, 104)));
        movies.add(new Movies(3, "Vikram", "2h 55m", "Action", "Tamil", Arrays.asList(102, 103)));
        movies.add(new Movies(4, "RRR", "3h 2m", "Action", "Telugu", Arrays.asList(101, 104, 105)));
        movies.add(new Movies(5, "Spirited Away", "2h 5m", "Fantasy", "Japanese", Arrays.asList(102, 105)));
        movies.add(new Movies(6, "Bahubali", "2h 40m", "Historical", "Telugu", Arrays.asList(101, 103, 105)));
        movies.add(new Movies(7, "Interstellar", "2h 49m", "Sci-Fi", "English", Arrays.asList(102, 104)));
        movies.add(new Movies(8, "Jawan", "2h 30m", "Action", "Hindi", Arrays.asList(101, 102, 103)));

	}

	public void displayAllMovie(boolean callFeatures) {
	    System.out.println("--------------------------------------------------------------------------------");
	    System.out.printf("| %-4s | %-15s |%-8s | %-10s | %-10s | %-15s |\n", 
	                      "ID", "Title", "Duration", "Genre", "Language", "Theatres");
	    System.out.println("--------------------------------------------------------------------------------");

	    for (Movies movie : movies) {
	        String theatres = movie.listTheatre.toString().replace("[", "").replace("]", "");
	        System.out.printf("| %-4d | %-15s |%-8s | %-10s | %-10s | %-15s |\n",
	                          movie.getMovieId(), movie.getTitle(), movie.getDuration(), 
	                          movie.getGenre(), movie.getLanguage(), theatres);
	    }
